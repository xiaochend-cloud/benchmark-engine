...
"""AE-only training pipeline for Duo Authentication logs (benchmark engine variant)."""
...
def run_pipeline(source,
                 ...
                 file_type_override,
                 **kwargs):
    """Runs the AE-only training pipeline."""
    ...
    logger.info("Running AE training pipeline")
    ...
    pipeline.add_stage(AutoEncoderTraining(config, validation_size=0.10))

    pipeline.add_stage(MonitorStage(config, description="AE Training rate", smoothing=0.001))
