...
"""GNN-only malware detection pipeline for the CopperDroid dataset.
...
@click.option("--dropout", type=float, default=0.5, show_default=True, help="Dropout probability.")
...
def run_pipeline(
    csv_path, label_column, val_size, test_size, split_seed, k,
    epochs, lr, hidden_dim, dropout, weight_seed, device,
    experiment_name, tracking_uri, log_level,
):
    """GNN-only malware classification pipeline for the CopperDroid dataset."""
    configure_logging(log_level=log_level)
    logging.getLogger("mlflow").setLevel(log_level)

    mlflow.set_tracking_uri(tracking_uri)
    progress_tracker.init("copperdroid-gnn")
    ...
    pipeline.add_stage(GraphConstructionStage(config, k=k))
    ...
    pipeline.add_stage(MonitorStage(config, description="GNN training rate", smoothing=0.001))
...
if __name__ == "__main__":
    run_pipeline(obj={}, auto_envvar_prefix="COPPERDROID", show_default=True, prog_name="copperdroid-gnn")
