...
class PipelineMonitor(App):
    ...
    def _render_training(self, key: str, title: str, stages: dict) -> str:
        ...
        if status == "completed" and s.get("metrics"):
            ...
            lines += "\n[bold]Compute[/bold]\n"
            wall = s.get("wall_time_s")
            if wall is not None:
                mins, secs = divmod(int(wall), 60)
                wall_str = f"{mins:02d}:{secs:02d}" if mins else f"{wall:.1f}s"
                lines += f"  {'Wall time':<12} {wall_str}\n"
            gpu_mem = s.get("peak_gpu_mem_mib")
            if gpu_mem is not None:
                lines += f"  {'Peak GPU':<12} {gpu_mem:.1f} MiB\n"
            gpu_util = s.get("avg_gpu_util_pct")
            if gpu_util is not None:
                lines += f"  {'Avg GPU %':<12} {gpu_util:.1f}%\n"
            if wall is None and gpu_mem is None and gpu_util is None:
                lines += "  [dim]No compute data[/dim]\n"
